...
% Date: 08.14.2019
% Rev for allSubjects: 08.28.2019
%
% Rev 1 dated 08.29.2019
% This file will first import each file individually, and then combine the
% analysis into a larger array, instead of importing everything first into
% a giant array.
%
% Rev 2 dated 09.05.2019
% Incorporates analysis for:
% Result 0 - percent correct in terms of step longer and shorter
...
if 0    % NOT PLOTTING THIS
% Plotting of percent correct for T, T+1, and T+2
% Tutorial from https://stackoverflow.com/questions/29155899/matlab-multipleparallel-box-plots-in-single-figure
figure;
N = 3;
delta = linspace(-.3,.3,N); %// define offsets to distinguish plots
width = .3; %// small width to avoid overlap
groupLabel = {'T', 'T+1', 'T+2'};
for i = 1:1:6
    fname = ['s',num2str(i)];
    f1 = subplot(3,2,i,'position',posMatrix(i,:)); hold on;
    boxplot([sCorCount.(fname)(1,:)' sCorCount.(fname)(2,:)' sCorCount.(fname)(3,:)'], ...
        'Colors',purple,'position',(1:N)+delta(1),'widths',width,'labels', groupLabel,'boxstyle','filled','Symbol','kx');
    plot(NaN,1,'color',purple); % Dummy plot for legend
    boxplot([lCorCount.(fname)(1,:)' lCorCount.(fname)(2,:)' lCorCount.(fname)(3,:)'], ...
        'Color',darkGold,'position',(1:N)+delta(2),'widths',width,'labels', groupLabel,'boxstyle','filled','Symbol','kx');
    plot(NaN,1,'color',darkGold); % Dummy plot for legend
    plot([0 100],[50 50],'--r');
   
    % Legend
    lgd = legend('Short','Long');
    lgd.NumColumns = 2;
    lgd.TextColor = purple;
    ...
    % Title and axes
    ylim([-25 125]);
    ylabel('Correct stride change (%)');
    title(['ST', num2str(i)], 'Color', purple);
    f1x = ancestor(f1, 'axes');
    f1x.TitleFontWeight = 'normal';
    f1x.FontName = 'Uni Sans Regular';
    f1x.FontSize = 10;
    ...
    % Global title
    annotation('textbox', [0 1 1 0],'String', 'Percentage of correct directional stride length changes (n=10)',...
        'EdgeColor','none','HorizontalAlignment', 'center',...
        'FontName','Uni Sans Regular','FontSize',15,'Color',purple); 
end   
end

%% Result 0b - T-test for percent correct

TTRes0 = zeros(4,6);
nanV = zeros(30,1);
nanV = NaN.*nanV;
for i = 1:1:6
   [h,p] = ttest2(zCorCountFlat, simpleSCC(:,i));
   TTRes0(1:2,i) = [h; p];
   
   [h,p] = ttest2(zCorCountFlat, simpleLCC(:,i));
   TTRes0(3:4,i) = [h; p];
end

TTRes0
% for i = 1:1:6
%    [h,p] = signrank(zCorCountFlat, [simpleSCC(:,i); nanV]);
%    TTRes0(1:2,i) = [h; p];
%    
%    [h,p] = signrank(zCorCountFlat, [simpleLCC(:,i); nanV]);
%    TTRes0(3:4,i) = [h; p];
% end
...
% Aggregate notboxplot
figure;
for i = 1:1:6
    fname = ['s',num2str(i)];
    %  plot the subplot
    f1 = subplot(3,2,i,'position',posMatrix3by2(i,:));
    plot([0 21],[1 1],'--r','Color',red); hold on;
    %boxplot(eval(['target.s', num2str(i),'s']), 'Colors', purple, 'OutlierSize', 6);
    h = notBoxPlot(aggNormAll.(fname),'jitter',0.5,'markMedian',true); 
    set([h([1,4,7,10,13]).data],'markerfacecolor',purple,'markeredgecolor','none','markersize',1.5);
    set([h([1,4,7,10,13]).semPtch],'FaceColor','none','EdgeColor','none');
    set([h([1,4,7,10,13]).sdPtch],'FaceColor','none','EdgeColor','none');
    set([h([1,4,7,10,13]).med],'Color',purple);
    set([h([1,4,7,10,13]).mu],'Color','none');
    set([h([2,5,8,11,14]).data],'markerfacecolor',xDarkGold,'markeredgecolor','none','markersize',1.5);
    set([h([2,5,8,11,14]).semPtch],'FaceColor','none','EdgeColor','none');
    set([h([2,5,8,11,14]).sdPtch],'FaceColor','none','EdgeColor','none');
    set([h([2,5,8,11,14]).med],'Color',xDarkGold);
    set([h([2,5,8,11,14]).mu],'Color','none');
    
    % Legend
    lgd = legend([h(1).med,h(2).med],...
        'Step Shorter Median', 'Step Longer Median');
    lgd.NumColumns = 1;
    lgd.TextColor = purple;
    ...
    xticklabels({'T-2','T-1','Target (T)','T+1','T+2'});
    ylim([0.2 1.6]);
    ylabel('Normalized right stride length');
    title(['Stim Time (ST) ', num2str(i)], 'Color', purple);
    f1x = ancestor(f1, 'axes');
    f1x.TitleFontWeight = 'normal';
    f1x.FontName = 'Uni Sans Regular';
    f1x.FontSize = 10;
    ...
    % Global title
    annotation('textbox', [0 1 1 0],'String', 'Normalized right stride length for n=10 (scatter and median)',...
        'EdgeColor','none','HorizontalAlignment', 'center',...
        'FontName','Uni Sans Regular','FontSize',15,'Color',purple);
end

% % Aggregate Brittney notboxplot
% figure;
% for i = 1:1:6
%     fname = ['s',num2str(i)];
%     %  plot the subplot
%     f1 = subplot(3,2,i,'position',posMatrix(i,:));
%     plot([0 21],[1 1],'--r','Color',red); hold on;
%     %boxplot(eval(['target.s', num2str(i),'s']), 'Colors', purple, 'OutlierSize', 6);
%     h = notBoxPlot(aggNormAll.(fname),'jitter',0.5,'markMedian',true); 
%     set([h([1,4,7,10,13]).data],'markerfacecolor','none','markeredgecolor','none','markersize',1.5);
%     set([h([1,4,7,10,13]).semPtch],'FaceColor','none','EdgeColor','none');
%     set([h([1,4,7,10,13]).sdPtch],'FaceColor',darkPurple,'EdgeColor','none');
%     set([h([1,4,7,10,13]).med],'Color','none');
%     set([h([1,4,7,10,13]).mu],'Color',purple);
%     set([h([2,5,8,11,14]).data],'markerfacecolor','none','markeredgecolor','none','markersize',1.5);
%     set([h([2,5,8,11,14]).semPtch],'FaceColor','none','EdgeColor','none');
%     set([h([2,5,8,11,14]).sdPtch],'FaceColor',darkGold,'EdgeColor','none');
%     set([h([2,5,8,11,14]).med],'Color','none');
%     set([h([2,5,8,11,14]).mu],'Color',xDarkGold);
%     
%     % Legend
%     lgd = legend([h(1).sdPtch,h(1).mu,h(2).sdPtch,h(2).mu],...
%         'Step Shorter SD', 'Step Shorter Mean', 'Step Longer SD', 'Step Longer Mean');
%     lgd.NumColumns = 1;
%     lgd.TextColor = purple;
%     lgd.Location = 'NorthWest';
%    
...
%     
%     % Global title
%     annotation('textbox', [0 1 1 0],'String', 'Normalized right stride length for n=10 (mean and standard deviation (SD))',...
%         'EdgeColor','none','HorizontalAlignment', 'center',...
%         'FontName','Uni Sans Regular','FontSize',15,'Color',purple);
% end
...
% S0 for each individuals
figure;
for i = numSubject/2+1:1:numSubject %1:1:numSubject/2 % 
    f1 = subplot(numSubject/2,1,i-5)
    p1 = plot([0 5],[mean(cell2mat(s0mean(i))) mean(cell2mat(s0mean(i)))],'--r','Color',red); hold on;
    
    boxplot(s0{i},'Colors',purple,'widths',width,'boxstyle','filled','Symbol','kx');
   
    % Title and axes
    title(['User ', num2str(i)]); %, 'Color', purple);
...
%     f1x.XColor = purple;
    ...
    % Global title
    annotation('textbox', [0 1 1 0],'String', 'Right stride length for ST0',...
        'EdgeColor','none','HorizontalAlignment', 'center',...
        'FontName','Uni Sans Regular','FontSize',15); %,'Color',purple); 
end

%% Result 5b - Plot speed
figure;
for i = 1:1:numSubject
    % Aggreggating speed long and short into a single array for grouped
    % boxplot
    nanV = zeros(size(speedShort{i}(:,1),1),size(speedShort{i}(:,1),2));
    nanV = NaN.*nanV;
    speedAggregate = [speedShort{i}(:,1) speedLong{i}(:,1) nanV ...
        speedShort{i}(:,2) speedLong{i}(:,2) nanV ...
        speedShort{i}(:,3) speedLong{i}(:,3) nanV ...
        speedShort{i}(:,4) speedLong{i}(:,4) nanV ...
        speedShort{i}(:,5) speedLong{i}(:,5) nanV ...
        speedShort{i}(:,6) speedLong{i}(:,6)];
    
    % notBoxPlot
    f1 = subplot(numSubject/2,2,i);
    p1 = plot([0 18],[s0SpeedMean(i) s0SpeedMean(i)],'--r','Color',red); hold on;
    h = notBoxPlot(speedAggregate,'jitter',0.3,'markMedian',true);
    set([h([1,4,7,10,13,16]).data],'markerfacecolor',red,'markeredgecolor','none','markersize',2);
    set([h([1,4,7,10,13,16]).semPtch],'FaceColor',lightPurple,'EdgeColor','none');
    set([h([1,4,7,10,13,16]).sdPtch],'FaceColor',darkPurple,'EdgeColor','none');
    set([h([1,4,7,10,13,16]).med],'Color',purple);
    set([h([1,4,7,10,13,16]).mu],'Color',purple);
    set([h([2,5,8,11,14,17]).data],'markerfacecolor',red,'markeredgecolor','none','markersize',2);
    set([h([2,5,8,11,14,17]).semPtch],'FaceColor',lightGold,'EdgeColor','none');
    set([h([2,5,8,11,14,17]).sdPtch],'FaceColor',darkGold,'EdgeColor','none');
    set([h([2,5,8,11,14,17]).med],'Color',xDarkGold);
    set([h([2,5,8,11,14,17]).mu],'Color',xDarkGold);
    
    % Legend
    lgd = legend([p1 h(1).sdPtch,h(1).semPtch,h(1).mu,h(1).med,...
        h(2).sdPtch,h(2).semPtch,h(2).mu,h(2).med],...
        'No Stim Mean','Short SD','Short SEM','Short Mean','Short Median',...
        'Long SD','Long SEM','Long Mean','Long Median');
    lgd.NumColumns = 2;
    lgd.TextColor = purple;
   
    % Title and axes
    title(['Participant ', num2str(i)], 'Color', purple);
    ylim([0.85 2]);
    xticks([1.5 4.5 7.5 10.5 13.5 16.5]); xticklabels({'S1','S2','S3','S4','S5','S6'});
    ylabel('Speed (m/s)');
    ...
    % Global title
    annotation('textbox', [0 1 1 0],'String', 'Speed (m/s) for n=10 across S0-S6',...
        'EdgeColor','none','HorizontalAlignment', 'center',...
        'FontName','Uni Sans Regular','FontSize',12,'Color',purple);
    
%     f1 = subplot(3,2,i*2-1);
%     %boxplot(target.s0, 'Colors', purple, 'OutlierSize', 6);
%     h = notBoxPlot(speedShort{i},'jitter',0.3,'markMedian',true);
%     set([h.data],'markerfacecolor',grey,'markeredgecolor','none','markersize',2);
%     set([h.semPtch],'FaceColor',lightPurple,'EdgeColor','none');
%     set([h.sdPtch],'FaceColor',gold,'EdgeColor','none');
%     set([h.mu],'Color',purple);
%     ylim([1 1.5]);
%     title(['Subject ', num2str(i), ' Speed Short'], 'Color', purple);
%     f1x = ancestor(f1, 'axes');
%     f1x.FontName = 'Uni Sans Regular';
%     f1x.FontSize = 8;
%     f1x.YColor = purple;
...
%     f1 = subplot(3,2,i*2);
%     %boxplot(target.s0, 'Colors', purple, 'OutlierSize', 6);
%     h = notBoxPlot(speedLong{i},'jitter',0.3,'markMedian',true);
%     set([h.data],'markerfacecolor',grey,'markeredgecolor','none','markersize',2);
%     set([h.semPtch],'FaceColor',lightPurple,'EdgeColor','none');
%     set([h.sdPtch],'FaceColor',gold,'EdgeColor','none');
%     set([h.mu],'Color',purple);
%     ylim([1 1.5]);
%     title(['Subject ', num2str(i), ' Speed Long'], 'Color', purple);
%     f1x = ancestor(f1, 'axes');
%     f1x.FontName = 'Uni Sans Regular';
%     f1x.FontSize = 8;
%     f1x.YColor = purple;
...
% Normalized mean stride length of S1-S6 vs average S0 speed
figure;
for i = 1:1:6
   fname = ['s',num2str(i)];
   subplot(3,2,i);
   plot(s0SpeedMean, normSTargetStride(:,i),'.', s0SpeedMean, normLTargetStride(:,i),'.');
   hold on;
   plot([0.95 1.5],[1 1],'--r');
end

% Absolute mean stride length for S1-S6 vs average S0 speed
for si = 1:1:10
    for i = 1:1:6
        fname = ['s',num2str(i)];
        ts(si,i) = targetStride.(fname)(si,1);
        tl(si,i) = targetStride.(fname)(si,2);
    end
    meants(si) = mean(ts(si,:));
    meantl(si) = mean(tl(si,:));
end
...
for i = 1:1:6
   fname = ['s',num2str(i)];
   subplot(3,2,i);
   plot(s0SpeedMean, ts(:,i),'.', s0SpeedMean, tl(:,i),'.');
   hold on;
end

% Absolute delta stride length of step long and step short for S1-S6 vs
% average S0 mean speed
figure;
for i = 1:1:6
   fname = ['s',num2str(i)];
   subplot(3,2,i);
   plot(s0SpeedMean, (tl(:,i) - ts(:,i)),'.');
   p = polyfit(s0SpeedMean', (tl(:,i) - ts(:,i)), 2);
   y = polyval(p, s0SpeedMean');
   f = fit(s0SpeedMean', (tl(:,i) - ts(:,i)), 'poly1');
   fitlm(s0SpeedMean', (tl(:,i) - ts(:,i)))
   hold on;
   plot(f, s0SpeedMean', (tl(:,i) - ts(:,i)));
%    plot(s0SpeedMean',y,'-g');
   plot([0.95 1.4],[0 0],'--r');
   
   title(['ST',num2str(i)]);
   ylim([-100 600]);
   legend('Datapoint for 1 participant');
   ylabel('(Stride Long - Stride Short)(mm)');
   xlabel('Average speed (m/s)');
end

%% Speed analysis for all trials instead of just the mean
...
figure;
h = notBoxPlot(qAll,'jitter',0.4,'markMedian',true);
set([h(1).data],'markerfacecolor',grey,'markeredgecolor','none','markersize',2);
set([h(1).semPtch],'FaceColor',grey,'EdgeColor','none');
set([h(1).sdPtch],'FaceColor','none','EdgeColor','none');
set([h(1).mu],'Color',purple);
set([h(2:7).data],'markerfacecolor',grey,'markeredgecolor','none','markersize',2);
set([h(2:7).semPtch],'FaceColor','g','EdgeColor','none');
set([h(2:7).sdPtch],'FaceColor','none','EdgeColor','none');
set([h(2:7).mu],'Color',purple);
